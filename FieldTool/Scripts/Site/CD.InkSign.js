    ...
    init: function ()
    {
        ...
        $("#savepng_InkSign").click(function (event) {
            if (CD.InkSign.signaturePad.isEmpty()) {
                ...
                $("#SafetyOfficerSignature").src = dataURL;
                var reportId = $("#reportID").val();
                var safetyofficerSignature = reportId + "_OfficerSignature";

                localforage.setItem(safetyofficerSignature, { safetyOfficerSignature: dataURL, reportID: reportId }).then(
                           function success() {
                               Helpers.showNotificationModal("Safety Officer Signature is Saved",
                   "Safety Officer Signature is saved!");
