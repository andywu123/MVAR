...
(function ($) {
    ...
    Busy.prototype.buildImg = function () {
        var html = "<img class='busyimg' src='" + this.options.img + "' alt='" + this.options.alt + "' title='" + this.options.title + "'";
    ...
    Busy.prototype.positionImg = function (target, busyImg, position) {
        ...
        var busyLeft;
        if (position == "left") {
            busyLeft = targetPosition.left - busyWidth - this.options.offset;
        }
        else if (position == "right") {
            busyLeft = targetPosition.left + targetWidth + this.options.offset;
        }
        else {
            busyLeft = targetPosition.left + (targetWidth - busyWidth) / 2.0;
        }
